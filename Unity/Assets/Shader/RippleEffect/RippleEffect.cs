...
public class RippleEffect : MonoBehaviour
{
    ...
    class Droplet
    {
        ...
        public void Reset(Vector2 setPosition)
        {
    ...
    float timer;
    int dropCount;
    public bool passiveRipple = false;
    ...
    void Update()
    {
        if (dropInterval > 0 && passiveRipple)
        {
    ...
    public void Emit(Vector2 setPosition)
    {
        //Camera camera = GetComponent<Camera>();
        setPosition.x /= Screen.width;
        setPosition.y /= Screen.height;

        droplets[dropCount++ % droplets.Length].Reset(setPosition);
    }
}
